...
def init_suff_stats_func(prior_params, L, mode):
    if mode == 'poisson':
        ...
        if 'lam_b_pri' not in prior_params.keys():
            prior_params['lam_b_pri'] = gamma(prior_params['lam_b_hyper_pri_shape'],
                                              1 / (prior_params['lam_b_hyper_pri_rate']), size=prior_params['m_multi'])

        suff_stats['ysum'] = np.tile(prior_params['lam_a_pri'], [L, prior_params['m_multi']])
        suff_stats['ycnt'] = np.tile(prior_params['lam_b_pri'], [L, 1])

    if mode == 'ar':
        suff_stats = {'s_ybar_ybar_inv': [], 's_y_y_plus_s0': [], 's_y_ybar': [], 's_y_cond_ybar_plus_s0': [],
                      'dff': [], 'V0_inv': []}
        ...
        suff_stats['s_y_y_plus_s0'] = np.tile(
            prior_params['S0'] + np.matmul(np.matmul(prior_params['M0'], V0_inv), prior_params['M0'].T), [L, 1, 1])
...
def update_suff_stats_base(yt, zt, L, suff_stats_base, mode):
    ...
    if mode == 'ar':
        if not bool(suff_stats_base):
            suff_stats_base = {'ybar_ybar': np.zeros((L, m_multi, m_multi)), 'y_y': np.zeros((L, m_multi, m_multi)),
                               'y_ybar': np.zeros((L, m_multi, m_multi)), 'ycnt': np.zeros(L)}
        for ii in range(L):
...
def sample_lik_params(suff_stats, mode):
    ...
    if mode == 'ar':
        ...
        sigma_mat_post = np.array(
            [ss.invwishart.rvs(df=suff_stats['dff'][ik], scale=suff_stats['s_y_cond_ybar_plus_s0'][ik]) for ik in
             range(L)])  # L-d-d mat
...
def compute_lik(yt, lik_params, mode):
    ...
    if mode == 'ar':
        ...
        for ik in range(L):
            for it in range(T - 1):
                lik_mat[it + 1, ik] = ss.multivariate_normal.pdf(yt[it + 1], mean=mun[ik, it],
                                                                 cov=lik_params['sigma_mat_post'][ik])
    return lik_mat
...
def update_suff_stats(init_suff_stats, suff_stats_base, mode, L):
    ...
    if mode == 'ar':
        ...
        for ii in range(L):
            ...
            suff_stats['s_y_cond_ybar_plus_s0'][ii] = suff_stats['s_y_y_plus_s0'][ii] - np.matmul(
                np.matmul(suff_stats['s_y_ybar'][ii], suff_stats['s_ybar_ybar_inv'][ii]), suff_stats['s_y_ybar'][ii].T)
            suff_stats['dff'][ii] += suff_stats_base['ycnt'][ii]
    return suff_stats


def init_gibbs_from_input_fmp(p, v0_range, v1_range, alpha0_a_pri, alpha0_b_pri, gamma0_a_pri, gamma0_b_pri,
                              prior_params, L, yt_all_ls, zt_input_all_ls, pi_bar, pi_init, mode, n_cores):
...
def init_gibbs_full_bayesian(p, v0_range, v1_range, alpha0_a_pri, alpha0_b_pri, gamma0_a_pri, gamma0_b_pri,
                             prior_params, L, mode):
    alpha0 = gamma(alpha0_a_pri, 1 / alpha0_b_pri)
    gamma0 = gamma(gamma0_a_pri, 1 / gamma0_b_pri)
    alpha0_init = gamma(alpha0_a_pri, 1 / alpha0_b_pri)
...
def sample_zw(pi_bar, kappa_vec, pi_init, L, lik_params, mode, yt_ls):
    ...
    for yt in yt_ls:
        ...
        # print(time.time()-start)
    return {'n_mat': n_mat, 'n_ft': n_ft, 'num_1_vec': num_1_vec, 'num_0_vec': num_0_vec,
            'suff_stats_base': suff_stats_base, 'uniq': uniq, 'zt': zt_all, 'wt': wt_all}
...
def sample_zw_fmp(yt_all_ls, pi_bar, kappa_vec, pi_init, L, lik_params, mode, init_suff_stats, n_cores):
    ...
    zt, wt, n_mat, num_1_vec, num_0_vec, n_ft, K, uniq, suff_stats = update_all_stats(results, init_suff_stats, mode,
                                                                                      L)
    return zt, wt, n_mat, num_1_vec, num_0_vec, n_ft, K, uniq, suff_stats
...
def sample_alpha(m_mat, n_mat, alpha0, m_init, n_ft, alpha0_init, alpha0_a_pri,
                 alpha0_b_pri):  ## first time point will definitely add one more table no matter what alpha is, so it won't affect alpha posterior
    ...
    alpha0 = gamma(alpha0_a_pri + (m_mat.sum()) - sum(s_vec),
                   1 / (alpha0_b_pri - sum(np.log(r_vec + eps))))  ## not consider first time point
...
def compute_rho_posterior(rho0, rho1, num_1_vec, num_0_vec):
    L = len(num_1_vec)
    log_posterior = L * (ssp.loggamma(rho0 + rho1) - ssp.loggamma(rho0) - ssp.loggamma(rho1)) + sum(
        ssp.loggamma(rho0 + num_1_vec)) + sum(ssp.loggamma(rho1 + num_0_vec)) - sum(
        ssp.loggamma(rho0 + rho1 + num_1_vec + num_0_vec))
...
def sample_lam_b_pri(lik_params, prior_params, uniq, K):
    prior_params['lam_b_pri'] = gamma(prior_params['lam_b_hyper_pri_shape'] + K * prior_params['lam_a_pri'], 1 / (
            prior_params['lam_b_hyper_pri_rate'] + (lik_params['lam_post'][np.array(uniq, dtype='int')]).sum(
        axis=0)))
    return prior_params
