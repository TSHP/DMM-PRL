...
def compute_log_marginal_lik_poisson(K, yt, zt, prob_mat, lam_a_pri, lam_b_pri, ysum, ycnt):
    ...
    for t in range(T):
        if t == 0 and zt == -1:
            j = 0
            a_mat[t + 1, j] = np.exp(
                np.sum(np.log(ss.nbinom.pmf(yt[t], lam_a_post[j], lam_b_post[j] / (lam_b_post[j] + 1)))))
        else:
            for j in range(K + 1):
...
def compute_log_marginal_lik_ar(K,yt,zt,prob_mat,M0,V0,S0,n0,s_ybar_ybar_inv,s_y_y_plus_s0,s_y_ybar,s_y_cond_ybar_plus_s0,dff):
    ...
    for t in range(1, T):
        ...
        yt_dist = np.array([multivariate_t_distribution(yt[t], mun[ik],
                                                        s_y_cond_ybar_plus_s0[ik] * (1 + (1 / nun[ik])) / dff[ik],
                                                        dff[ik]) for ik in range(K)])

        mu_new = np.matmul(M0, yt[t - 1])
        nu_new = 1 / np.sum(np.matmul(V0, yt[t - 1]) * yt[t - 1])
        yt_knew_dist = multivariate_t_distribution(yt[t], mu_new, S0 * (1 + (1 / nu_new)) / (n0 + 1 - m_multi),
                                                   (n0 + 1 - m_multi))
...
def compute_log_marginal_lik_ar_approx(L, yt, zt, prob_mat, suff_stats):
    ...
    for t in range(1, T):
        ...
        yt_dist = np.array([multivariate_t_distribution(yt[t], mun[ik],
                                                        suff_stats['s_y_cond_ybar_plus_s0'][ik] * (1 + (1 / nun[ik])) /
                                                        dff[ik], dff[ik]) for ik in range(L)])
...
def compute_log_marginal_lik_ar_approx_parallel(L, prob_mat, pi_init, suff_stats, yt_ls):
    ...
    for yt in yt_ls:
        ...
        for t in range(1, T):
            ...
            mun = np.array([np.matmul(suff_stats['s_y_ybar'][ik], tmp[ik]) for ik in range(L)])  # K by m_multi mat
            dff = suff_stats['dff'] + 1 - m_multi
            yt_dist = np.array([multivariate_t_distribution(yt[t], mun[ik], suff_stats['s_y_cond_ybar_plus_s0'][ik] * (
                        1 + (1 / nun[ik])) / dff[ik], dff[ik]) for ik in range(L)])
