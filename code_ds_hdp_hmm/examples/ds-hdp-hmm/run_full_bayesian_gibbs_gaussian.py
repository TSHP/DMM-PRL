...
for it in range(iters):
    if it == 0:
        rho0, rho1, alpha0, gamma0, sigma0, mu0, sigma0_pri, K, zt, wt, beta_vec, beta_new, kappa_vec, kappa_new, n_mat, ysum, ycnt = init_gibbs_full_bayesian(
            p, v0_range, v1_range, alpha0_a_pri, alpha0_b_pri, gamma0_a_pri, gamma0_b_pri, sigma0, mu0, sigma0_pri, T,
            yt_real)
    else:
        zt, wt, n_mat, ysum, ycnt, beta_vec, kappa_vec, beta_new, kappa_new, K = sample_zw(zt, wt, yt_real, n_mat, ysum,
                                                                                           ycnt, beta_vec, beta_new,
                                                                                           kappa_vec, kappa_new, alpha0,
                                                                                           gamma0, sigma0, mu0,
...
np.savez(rlt_path + file_name + '_full_bayesian_rlt_' + str(seed) + '.npz', zt=zt_sample, wt=wt_sample,
         kappa=kappa_vec_sample, hyper=hyperparam_sample, hamming=mismatch_vec, zt_permute=zt_sample_permute,
