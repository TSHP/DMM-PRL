...
def sample_one_step_ahead(zt, wt, yt, n_mat, ysum, ycnt, beta_vec, beta_new, kappa_vec, kappa_new, alpha0, gamma0,
                          lam_a_pri, lam_b_pri, rho0, rho1, K):
    T = len(zt)
    m_multi = len(yt[0])
    ...
    for t in range(1, T):
        ...
        post_cases = np.hstack((kappa_vec[j] * yt_dist[j], (1 - kappa_vec[j]) * zt_dist * yt_dist,
...
def init_gibbs(rho0, rho1, alpha0, gamma0, lam_a_pri, lam_b_pri, T, yt):
    ...
    zt, wt, n_mat, ysum, ycnt, beta_vec, kappa_vec, beta_new, kappa_new, K = sample_one_step_ahead(zt, wt, yt, n_mat,
                                                                                                   ysum, ycnt, beta_vec,
                                                                                                   beta_new, kappa_vec,
                                                                                                   kappa_new, alpha0,
                                                                                                   gamma0, lam_a_pri,
...
def init_gibbs_full_bayesian(p, v0_range, v1_range, alpha0_a_pri, alpha0_b_pri, gamma0_a_pri, gamma0_b_pri, lam_a_pri,
                             lam_b_hyper_pri_shape, lam_b_hyper_pri_rate, T, yt):
    K = 1
    zt = np.zeros(T, dtype='int')
...
def sample_last(zt, wt, yt, n_mat, ysum, ycnt, beta_vec, beta_new, kappa_vec, kappa_new, alpha0, gamma0, lam_a_pri,
                lam_b_pri, rho0, rho1, K):
    T = len(zt)
    m_multi = len(yt[0])
...
def sample_zw(zt, wt, yt, n_mat, ysum, ycnt, beta_vec, beta_new, kappa_vec, kappa_new, alpha0, gamma0, lam_a_pri,
              lam_b_pri, rho0, rho1, K):
    T = len(zt)
    m_multi = len(yt[0])
    ...
    for t in range(1, T - 1):
        ...
        tmp_vec = np.arange(K)
        zt_dist = (alpha0 * beta_vec + n_mat[j]) / (alpha0 + n_mat[j].sum())
        ztplus1_dist = (alpha0 * beta_vec[l] + n_mat[:, l] + (j == l) * (j == tmp_vec)) / (
        ...
        ## construct z,w's posterior by cases
        post_cases = np.array(((kappa_vec[j] ** 2) * yt_dist[j] * (j == l),
                               (1 - kappa_vec[j]) * kappa_vec[l] * zt_dist[l] * yt_dist[l],
                               kappa_vec[j] * (1 - kappa_vec[j]) * ztplus1_dist[j] * yt_dist[j]))
        post_cases = np.hstack((post_cases, (1 - kappa_vec[j]) * (1 - kappa_vec) * zt_dist * ztplus1_dist * yt_dist,
    ...
    zt, wt, n_mat, ysum, ycnt, beta_vec, kappa_vec, beta_new, kappa_new, K = sample_last(zt, wt, yt, n_mat, ysum, ycnt,
                                                                                         beta_vec, beta_new, kappa_vec,
                                                                                         kappa_new, alpha0, gamma0,
                                                                                         lam_a_pri, lam_b_pri, rho0,
                                                                                         rho1, K)
...
def compute_rho_posterior(rho0, rho1, K, num_1_vec, num_0_vec):
    log_posterior = K * (ssp.loggamma(rho0 + rho1) - ssp.loggamma(rho0) - ssp.loggamma(rho1)) + sum(
