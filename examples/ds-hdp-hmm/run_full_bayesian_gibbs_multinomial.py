...
for it in range(iters):
    if it == 0:
        rho0, rho1, alpha0, gamma0, dir0, K, zt, wt, beta_vec, beta_new, kappa_vec, kappa_new, n_mat, ysum = init_gibbs_full_bayesian(
            p, v0_range, v1_range, alpha0_a_pri, alpha0_b_pri, gamma0_a_pri, gamma0_b_pri, dir0, T, yt_real)
    else:
        zt, wt, n_mat, ysum, beta_vec, kappa_vec, beta_new, kappa_new, K = sample_zw(zt, wt, yt_real, n_mat, ysum,
                                                                                     beta_vec, beta_new, kappa_vec,
                                                                                     kappa_new, alpha0, gamma0, dir0,
                                                                                     rho0, rho1, K)
...
np.savez(rlt_path + file_name + '_full_bayesian_rlt_' + str(seed) + '.npz', zt=zt_sample, kappa=kappa_vec_sample,
         hyper=hyperparam_sample, hamming=mismatch_vec, zt_permute=zt_sample_permute,
         kappa_permute=kappa_sample_permute, loglik=loglik_test_sample, post=post_sample)
